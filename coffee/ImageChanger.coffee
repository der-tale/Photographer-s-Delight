# Base class for an image gallery. Creates markup and handles all callbacks to
# organize and display an image gallery. The animation is define by the extended
# class. It is up to the actual theme to further customize the markup/css and
...
class ImageChanger
    ...
    # ImageChanger takes a single object as an option value.
    #
    #     {
    ...
    display: (index, options) ->
        if options?.automaticAnimation then @automaticAnimation = true
        if not @images[index]
    ...
    # returns id of next image or -1 if no next image
    _animate: ->
        if @animationInProgress or @queue.length is 0 then return -1
